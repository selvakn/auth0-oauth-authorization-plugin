...
import cd.go.authorization.auth0.requests.VerifyConnectionRequest;
...
public class VerifyConnectionRequestExecutor implements RequestExecutor {
    ...
    private final Auth0ClientBuilder providerManager;

    public VerifyConnectionRequestExecutor(VerifyConnectionRequest request) {
        this(request, new Auth0ClientBuilder());
    }

    public VerifyConnectionRequestExecutor(VerifyConnectionRequest request, Auth0ClientBuilder providerManager) {
    ...
    private ValidationResult validate() {
        return new MetadataValidator().validate(request.config());
    }
